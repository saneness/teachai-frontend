<template>
  <div class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full flex justify-center items-center z-50" @click.self="handleBackdropClick">
      ...
      <form @submit.prevent="submitForm">
        ...
        <div class="mb-4">
          ...
          <div>
            <select
              id="programName"
              :value="editableClass.program_name"
    ...
    <!-- Confirmation Modal for Program Change -->
    ...
    <!-- Модальное окно подтверждения закрытия формы -->
...
const editableClass = ref({
  id: null, name: '', program_name: null, students: [], 
});
...
watch([editableClass, studentsText], () => {
    if (initialFormState.value.name !== undefined) {
      isDirty.value = true;
    }
}, { deep: true });
...
const submitForm = async () => {
  ...
  if (!props.isEditing && availablePrograms.value.length === 0 && !isProgramsLoading.value) { 
      formError.value = 'Cannot create class: No programs available.';
      return;
  }
